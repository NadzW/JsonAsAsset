				...
				if (paramsPtr.Num() == paramValuesPtr.Num()) {
					...
					for (const TSharedPtr<FJsonValue> paramVal : paramsPtr) {
						...
						if (
							...
							param->MaterialExpressionEditorX = x;
							param->MaterialExpressionEditorY = y;
							TObjectPtr<UTexture> tex = TSoftObjectPtr<UTexture>(FSoftObjectPath(static_cast<FTopLevelAssetPath>(FName(textureAssetPath)), textureSubPath)).LoadSynchronous();
							param->Texture = tex;
...
#pragma endregion param recreation
		...
		SavePackage(Material->GetPackage());
